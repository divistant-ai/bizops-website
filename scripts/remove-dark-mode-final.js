 ...
 *
 * Usage: node scripts/remove-dark-mode-final.js
 */
...
function removeDarkModeClasses(content) {
  ...
  // Remove dark: classes from className strings (double quotes)
  ...
  // Remove dark: classes from className strings (single quotes)
  modified = modified.replace(/className='([^']*)'/g, (match, classes) => {
    const cleaned = classes
      .split(/\s+/)
      .filter(cls => !cls.startsWith('dark:'))
  ...
  // Remove dark: from color definitions in data objects (but keep property names like "dark:")
  ...
  // Remove dark: from bg definitions in data objects
  modified = modified.replace(/bg:\s*["']([^"']*)["']/g, (match, classes) => {
    const cleaned = classes
      .split(/\s+/)
      .filter(cls => !cls.startsWith('dark:'))
      .join(' ')
      .trim();
    return cleaned ? `bg: "${cleaned}"` : 'bg: ""';
  });

  // Remove standalone dark: classes (not in className)
...
if (processed.length > 0) {
  ...
  processed.slice(0, 30).forEach(r => console.log(`  - ${r.file.replace(SRC_DIR, 'src')}`));
  if (processed.length > 30) {
    console.log(`  ... and ${processed.length - 30} more files`);
  }
}
